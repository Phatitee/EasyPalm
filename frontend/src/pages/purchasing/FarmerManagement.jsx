...
import { PlusCircle, Trash2, Pencil, User, Search, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
const FarmerManagement = () => {
    ...
    const [editingFarmer, setEditingFarmer] = useState({ f_id: null, f_name: '', f_citizen_id_card: '', f_tel: '', f_address: '' });
    ...
    // --- ดึงข้อมูล ---
    // Ensure fetchFarmers is defined within the component scope
    ...
    const handleCloseEditModal = () => {
        ...
        setEditingFarmer({ f_id: null, f_name: '', f_citizen_id_card: '', f_tel: '', f_address: '' });
        setEditFormErrors({}); // --- (เพิ่มใหม่) เคลียร์ Error เมื่อปิด
    };
    const handleCloseAddModal = () => {
        setAddModalOpen(false);
        // No need to clear form here if cleared on open
    };
    ...
    const handleEditFormChange = (e) => {
        ...
        // Apply similar input filtering as Add modal if needed
        ...
        setEditingFarmer(prev => ({ ...prev, [name]: processedValue }));
    ...
    const handleEditSubmit = async (e) => {
        ...
        if (!telPattern.test(editingFarmer.f_tel)) {
            errors.f_tel = 'กรุณากรอกเบอร์โทรศัพท์ 10 หลัก ที่ขึ้นต้นด้วย 0 ให้ถูกต้อง';
        }
        ...
        try {
            ...
            await axios.put(`http://127.0.0.1:5000/farmers/${editingFarmer.f_id}`, dataToUpdate);
            handleCloseEditModal();
            setModalInfo({ show: true, type: 'success', message: 'แก้ไขข้อมูลเกษตรกรสำเร็จ' });
            fetchFarmers();
        } catch (error) {
    ...
    const handleOpenAddModal = () => {
        setNewFarmerData({ f_name: '', f_citizen_id_card: '', f_tel: '', f_address: '' });
        setAddFormErrors({});
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            handleCloseAddModal();
            setModalInfo({ show: true, type: 'success', message: 'เพิ่มข้อมูลเกษตรกรสำเร็จ' });
            fetchFarmers();
        } catch (error) {
    ...
    const filteredFarmers = farmers.filter(farmer =>
        (farmer.f_name && farmer.f_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Check if f_name exists
        (farmer.f_tel && farmer.f_tel.toLowerCase().includes(searchTerm.toLowerCase()))     // Check if f_tel exists
    );
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 dark:bg-gray-900 dark:bg-opacity-75 flex justify-center items-center z-50 p-4">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl w-full max-w-lg">
                        ...
                        <FarmerForm
                            ...
                            onChange={handleEditFormChange} // ส่ง Handler เดิมไป
